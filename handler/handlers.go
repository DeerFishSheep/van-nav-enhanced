...
func GetAllHandler(c *gin.Context) {
	...
	subcatelogs := service.GetAllSubCatelog()
	if !utils.IsLogin(c) {
		// 过滤掉隐藏工具
...
func DeleteToolHandler(c *gin.Context) {
	...
	// 获取要删除的工具的Sort值
	var deletedSort int
	err = database.DB.QueryRow(`SELECT sort FROM nav_table WHERE id = ?`, numberId).Scan(&deletedSort)
	utils.CheckErr(err)
	...
	// 删除后，将后续工具的Sort前移（ID保持不变）
	err = database.ReorderToolsAfterDelete(deletedSort)
	utils.CheckErr(err)
...
func DeleteCatelogHandler(c *gin.Context) {
	...
	// 先获取分类名称
	var catelogName string
	err := database.DB.QueryRow(`SELECT name FROM nav_catelog WHERE id = ?`, id).Scan(&catelogName)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success":      false,
			"errorMessage": "查询分类信息失败",
		})
		return
	}
	...
	// 检查该分类下是否有书签
	var count int
	err = database.DB.QueryRow(`SELECT COUNT(*) FROM nav_table WHERE catelog = ?`, catelogName).Scan(&count)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success":      false,
			"errorMessage": "检查分类下书签数量失败",
		})
		return
	}
	...
	if count > 0 {
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
			"errorMessage": fmt.Sprintf("该分类下还有 %d 个书签，无法删除", count),
		})
		return
	}
	...
	// 获取被删除分类的 Sort 值
	var deletedSort int
	err = database.DB.QueryRow(`SELECT sort FROM nav_catelog WHERE id = ?`, id).Scan(&deletedSort)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"success":      false,
			"errorMessage": "查询分类排序信息失败",
		})
		return
	}
	...
	// 删除后，将后续大分类的Sort前移
	err = database.ReorderCatelogsAfterDelete(deletedSort)
	if err != nil {
		logger.LogError("删除大分类后重排序失败: %v", err)
	}
...
// 获取所有子分类
func GetAllSubCatelogsHandler(c *gin.Context) {
	subcatelogs := service.GetAllSubCatelog()
	c.JSON(200, gin.H{
		"success": true,
		"data":    subcatelogs,
...
// 根据大分类ID获取子分类
func GetSubCatelogsByCatelogHandler(c *gin.Context) {
	catelogIdStr := c.Param("catelogId")
	catelogId, err := strconv.Atoi(catelogIdStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
			"errorMessage": "无效的分类ID",
		})
		return
	}
	
	subcatelogs := service.GetSubCatelogsByCatelogId(catelogId)
	c.JSON(200, gin.H{
		"success": true,
		"data":    subcatelogs,
	})
...
// 添加子分类
func AddSubCatelogHandler(c *gin.Context) {
	var data types.AddSubCatelogDto
	if err := c.ShouldBindJSON(&data); err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
	...
	id, err := service.AddSubCatelog(data)
	if err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
...
// 更新子分类
func UpdateSubCatelogHandler(c *gin.Context) {
	var data types.UpdateSubCatelogDto
	if err := c.ShouldBindJSON(&data); err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
	...
	err = service.UpdateSubCatelog(data)
	if err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
...
// 删除子分类
func DeleteSubCatelogHandler(c *gin.Context) {
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
	...
	err = service.DeleteSubCatelog(id)
	if err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
...
// 更新子分类排序
func UpdateSubCatelogSortHandler(c *gin.Context) {
	var updates []types.UpdateSubCatelogsSortDto
	if err := c.ShouldBindJSON(&updates); err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusBadRequest, gin.H{
			"success":      false,
	...
	err := service.UpdateSubCatelogsSort(updates)
	if err != nil {
		utils.CheckErr(err)
		c.JSON(http.StatusInternalServerError, gin.H{
			"success":      false,
