...
func InitDB() {
	...
	// tools数据表结构升级-20250105-【子分类】
	if !columnExists("nav_table", "subcatelog_id") {
		DB.Exec(`ALTER TABLE nav_table ADD COLUMN subcatelog_id INTEGER DEFAULT 0;`)
	}
	...
	// 子分类表
	sql_create_table = `
		CREATE TABLE IF NOT EXISTS nav_subcatelog (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			name TEXT NOT NULL,
