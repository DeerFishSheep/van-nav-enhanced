...
// 获取所有子分类
export const fetchGetAllSubCatelogs = async () => {
    const { data } = await axios.get(`/api/admin/subcatelog`);
    return data?.data || [];
};

// 根据大分类ID获取子分类
export const fetchGetSubCatelogsByCatelogId = async (catelogId: number) => {
    const { data } = await axios.get(`/api/admin/subcatelog/${catelogId}`);
    return data?.data || [];
};

// 添加子分类
export const fetchAddSubCatelog = async (payload: any) => {
    const { data } = await axios.post(`/api/admin/subcatelog`, payload);
    return data?.data || {};
};

// 更新子分类
export const fetchUpdateSubCatelog = async (payload: any) => {
    const { data } = await axios.put(`/api/admin/subcatelog/${payload.id}`, payload);
    return data?.data || {};
};

// 删除子分类
export const fetchDeleteSubCatelog = async (id: number) => {
    const { data } = await axios.delete(`/api/admin/subcatelog/${id}`);
    return data?.data || {};
};

// 更新子分类排序
export const fetchUpdateSubCatelogsSort = async (updates: { id: number; sort: number }[]) => {
    const { data } = await axios.put(`/api/admin/subcatelogs/sort`, updates);
    return data?.data || {};
};